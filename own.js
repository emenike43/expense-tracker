...
opt.onchange = () =>{
    ...
    tinc.textContent = `${symbols}${number}`
    texp.textContent = `${symbols}${number}`
...
inc.onclick = () => {
    ...
    active = 'income'
  // make INC active
  inc.style.color = "#0F6E56"
  inc.style.backgroundColor = "#E1F5EE"
...
exp.onclick = () => {
    ...
    active = "expense"
  // make EXP active
  exp.style.color = "#993C1D"
  exp.style.backgroundColor = "#FFEAEA"
  ...
  if (active === "expense" && amount > currentBalance) {
    ...
    now.innerHTML = ` <div class="flex justify-between mb-2">
                     <div class="flex gap-3">
                          ...
                          <div>
                            <h class="text-[#1a1a1a] font-semibold">${desc.value}</h1>
